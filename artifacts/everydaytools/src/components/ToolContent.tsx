...
export function ToolLoadingState({ status, progress, label, errorMessage, onRetry }: ToolLoadingStateProps) {
    ...
    return (
      ...
      >
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true" style={{ flexShrink: 0, marginTop: 1 }}>
          <circle cx="8" cy="8" r="7.5" fill="rgba(239,68,68,0.12)" stroke="rgba(239,68,68,0.5)" />
          <path d="M8 4.5v4M8 10.5v.5" stroke="var(--danger)" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{
            fontFamily: 'var(--font-ui)', fontSize: 'var(--text-sm)',
            color: 'var(--danger)', margin: 0, lineHeight: 1.5,
  ...
  return (
    ...
    >
      {label && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          <svg
            ...
            aria-hidden="true"
            style={{ animation: 'tool-spin 0.8s linear infinite', flexShrink: 0 }}
          >
      ...
      <div
        role="progressbar"
        aria-valuenow={isIndeterminate ? undefined : Math.min(100, Math.max(0, progress!))}
        aria-valuemin={0}
        aria-valuemax={100}
        style={{
          height: 6,
      ...
      >
        {isIndeterminate ? (
          <div style={{
            ...
            borderRadius: 99,
            animation: 'tool-indeterminate 1.4s ease-in-out infinite',
          }} />
        ) : (
          <div style={{
            height: '100%',
