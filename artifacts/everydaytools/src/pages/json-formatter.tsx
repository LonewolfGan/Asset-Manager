        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 16 }}>
          ...
          <div>
            ...
            <textarea readOnly value={output} placeholder={t.common.outputAppearsHere}
              style={{ width: '100%', height: 420, padding: 14, border: '1px solid var(--border)', borderRadius: 'var(--radius)', background: 'var(--bg-elevated)', color: 'var(--text-primary)', fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', resize: 'vertical', boxSizing: 'border-box', lineHeight: 1.5 }}
            />
            {output && <p style={{ color: 'var(--text-tertiary)', fontSize: 'var(--text-xs)', marginTop: 6, fontFamily: 'var(--font-mono)' }}>{t.jsonFormatter.stats(output.length, new Blob([output]).size)}</p>}
          </div>
        </div>
